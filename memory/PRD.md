...
- 2026-06-19 — **Render keep-alive system + cold-start analysis** (validated via curl/pytest + workflow simulation):
  - **Root cause of the ~50s:** Render free-tier *container cold-start* (spin-up after 15 min idle) + MongoDB Atlas free-tier wake — NOT the app's startup. Confirmed the app does no GitHub API calls / file loads / blocking sync ops at boot; `on_startup` only runs idempotent index creation + a one-time admin seed + an (empty) slug backfill (all sub-second). Documented in `backend/KEEP_ALIVE.md` §1.
  - **GitHub Action** `.github/workflows/keep-alive.yml` — runs every 10 min (cron `*/10 * * * *`) + manual dispatch. Pings `/health` (falls back to `/api/health`), then best-effort warms `/api/posts`, `/api/case-studies`, `/api/videos`, `/api/works` to keep the Mongo pool + Atlas warm (benefits Admin dashboard too). Requires repo variable `BACKEND_URL`. Fails visibly if the backend is down; warm requests never fail the job.
  - **Optimisations:** added a bare dependency-free `GET /health → {"status":"ok"}` on the app (outside `/api`, ~2–3 ms) alongside existing `/api/health`; made `on_startup` non-blocking (`asyncio.create_task(_bootstrap())`) so the server is ready immediately instead of awaiting DB bootstrap. Safe — all idempotent + login has a bootstrap fallback.
  - **Before/After + validation** in `backend/KEEP_ALIVE.md` §4. The frontend memory-only SWR cache already masks any residual cold-start for returning visitors.
...
- 2026-06-17 — **Enhancement round (Services redesign, consultant CTA, site-wide hero imagery, content docs)** — frontend verified 100% (iteration_4.json), no backend changes:
  ...
  - **Site-wide hero background imagery.** New reusable `HeroImageLayer.jsx` (masked, grayscale, onError-safe). Applied to About, Services, Industries, Our Work, Contact, Blog and Case Studies heroes. Image URLs centralized in `PAGE_HERO_IMAGES` (content.js).
  - **Content extensibility + docs.** Service/Industry detail pages now render an optional `extraSections: [{heading, body}]` array (defined per object in content.js) — a documented, no-code-change way to add more copy. In-code `📝` markers added to content.js, ServiceDetail.jsx, IndustryDetail.jsx, TeamProfile.jsx. New **`/app/docs/CONTENT_EDITING_GUIDE.md`** maps exactly where to edit every kind of content (services/industries copy & images, team profiles via admin, blog/case-studies/videos via admin, hero images).
